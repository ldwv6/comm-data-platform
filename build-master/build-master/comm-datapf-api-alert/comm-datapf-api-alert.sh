...
IMAGE_NAME=comm-data-platform/$PROFILE/comm-datapf-api-alert
...
docker run -d $TERM_OPT --name $CONTAINER_NAME \
  ...
  -v /usr/local/comm-datapf-api-alert/config:/webapp/config \
  -v /usr/local/comm-datapf-api-alert/logs:/webapp/logs \
  -v /usr/local/comm-datapf-api-alert/gc:/webapp/gc \
  -p 18080:18080/tcp \
