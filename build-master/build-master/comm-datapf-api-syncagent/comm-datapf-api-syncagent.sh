...
IMAGE_NAME=comm-data-platform/$PROFILE/comm-datapf-api-syncagent
CONTAINER_NAME=comm-datapf-api-syncagent-$agent_div
...
docker run -d $TERM_OPT --name $CONTAINER_NAME \
  ...
  -v /usr/local/comm-datapf-api-syncagent/gc:/webapp/gc \
  -p $PORT:$PORT/tcp \
