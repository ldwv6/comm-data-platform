...
case $AGENT in
    shop)
        PORT=`expr 9500 + $div_topic_partition`
        break
        ;;
    book)
        PORT=`expr 9510 + $div_topic_partition`
        break
        ;;
    ticket)
        PORT=`expr 9520 + $div_topic_partition`
        break
        ;;
    movie)
        PORT=9530
...
IMAGE_NAME=comm-data-platform/$PROFILE/comm-datapf-api-kafkaconsumer
if [ "$AGENT" == "shop" -o "$AGENT" == "book" -o "$AGENT" == "ticket" ]; then
  CONTAINER_NAME=comm-datapf-api-kafkaconsumer-$agent_div$div_topic_partition
else
  CONTAINER_NAME=comm-datapf-api-kafkaconsumer-$agent_div
fi

echo "### NOW DEPLOY >>>> $CONTAINER_NAME ..........."
...
docker run -d $TERM_OPT --name $CONTAINER_NAME \
  ...
  -e agent_port=$PORT \
  -e div_topic_partition="$div_topic_partition" \
  ...
  -v /usr/local/comm-datapf-api-kafkaconsumer/gc:/webapp/gc \
  -p $PORT:$PORT/tcp \
